	...
	public static void main(String[] args) {
		...
		manage.deleteUser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (manage.idUser.getText().equals("")) {
					...
					int result = JOptionPane.showConfirmDialog(null, "Bạn có muốn xoá tài khoản này không?", "Xác nhận xoá tài khoản",
							JOptionPane.YES_NO_OPTION);
					if (result == JOptionPane.YES_OPTION) {
		...
		manage.deleteVehicle.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String id = manage.idVehicle.getText();
				if (id.equals("")) {
					manage.notification("Vui lòng chọn phương tiện");
				} else {
					int result = JOptionPane.showConfirmDialog(null, "Bạn muốn xoá phương tiện này không?", "Xác nhận xoá",
							JOptionPane.YES_NO_OPTION);
